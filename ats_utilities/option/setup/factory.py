    ...
    def create_bundle(cls, options: OptionOptions) -> OptionBundle:
        ...
        parser: ParserAdapter = ParserAdapter(
            parser=ArgumentParser(
                prog=f'{OptionKeys.GET_NAME(parameters)} {OptionKeys.GET_VERSION(parameters)}',
                epilog=f'{OptionKeys.GET_NAME(parameters)} copyright (c) {OptionKeys.GET_LICENCE(parameters)}',
                description=f'{OptionKeys.GET_NAME(parameters)} build date {OptionKeys.GET_BUILD_DATE(parameters)}'
            )
        )
