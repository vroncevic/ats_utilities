...
class CheckBaseConfig(object):
    ...
    __slots__ = ('VERBOSE', '__BASE_CONFIG', '__checker')
    ...
    def is_correct(self, configuration, verbose=False):
        """
        ...
        config_keys, statuses = configuration.keys(), []
        config_expected_keys = CheckBaseConfig.__BASE_CONFIG.values()
        verbose_message(
            CheckBaseConfig.VERBOSE, verbose,
            'Checking ATS configuration structure'
        )
        for cfg_key in config_keys:
            if cfg_key not in config_expected_keys:
                message = "{0} [{1}]".format('Key not expected', cfg_key)
                error_message(CheckBaseConfig.VERBOSE, message)
