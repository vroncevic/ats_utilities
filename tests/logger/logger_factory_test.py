...
class LoggerFactoryTest(unittest.TestCase):
    '''
        Defines class LoggerFactoryTest with attribute(s) and method(s).
        Tests LoggerFactory static factory logic.
    '''

    def test_create_default_logger_bundle(self) -> None:
        bundle = LoggerFactory.create_default_logger_bundle(
    ...
    @patch("ats_utilities.logger.logger_factory.getLogger")
    def test_create_default_logger_bundle_configures_stream(self, mock_get_logger) -> None:
        mock_logger = MagicMock()
        mock_logger.hasHandlers.return_value = False
        mock_get_logger.return_value = mock_logger

        with patch("ats_utilities.logger.logger_factory.basicConfig") as mock_basic_config:
            bundle = LoggerFactory.create_default_logger_bundle(log_file=None)
    ...
    def test_create_default_logger_bundle_configures_filename(self, mock_get_logger) -> None:
        ...
        with patch("ats_utilities.logger.logger_factory.basicConfig") as mock_basic_config:
            bundle = LoggerFactory.create_default_logger_bundle(log_file="test_file.log")
            mock_basic_config.assert_called_once()
            args, kwargs = mock_basic_config.call_args
