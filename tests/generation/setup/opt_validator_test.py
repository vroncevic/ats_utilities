...
Info
    Unit tests for GeneratorBundleOptionsValidator class.
'''
...
class GeneratorOptionsValidatorTest(unittest.TestCase):
    '''
        ...
        Tests GeneratorBundleOptionsValidator component logic.
    '''
    ...
    def test_validate_valid(self) -> None:
        options = GeneratorBundleOptions(context_bundle=self.mock_context)
        GeneratorBundleOptionsValidator.validate(options)

    def test_validate_invalid_none(self) -> None:
        with self.assertRaises(ATSValueError):
            GeneratorBundleOptionsValidator.validate(None)  # type: ignore

    def test_validate_invalid_type(self) -> None:
        with self.assertRaises(ATSTypeError):
            GeneratorBundleOptionsValidator.validate("invalid")  # type: ignore

    def test_validate_invalid_option_types(self) -> None:
        options = GeneratorBundleOptions(context_bundle="invalid")  # type: ignore
        with self.assertRaises(ATSTypeError):
            GeneratorBundleOptionsValidator.validate(options)

    @patch("ats_utilities.generation.setup.opt_validator.GeneratorBundleKeys.get_option_to_type")
    def test_validate_additional_options(self, mock_get_opt_to_type: MagicMock) -> None:
        ...
        options_none = GeneratorBundleOptions(context_bundle=self.mock_context)
        options_none["dummy_opt"] = None  # type: ignore
        GeneratorBundleOptionsValidator.validate(options_none)
        
        # When dummy_opt is not None (and matches type)
        options_val = GeneratorBundleOptions(context_bundle=self.mock_context)
        options_val["dummy_opt"] = "hello"  # type: ignore
        GeneratorBundleOptionsValidator.validate(options_val)

        # When dummy_opt has wrong type
        options_bad = GeneratorBundleOptions(context_bundle=self.mock_context)
        options_bad["dummy_opt"] = 123  # type: ignore
        with self.assertRaises(ATSTypeError):
            GeneratorBundleOptionsValidator.validate(options_bad)
