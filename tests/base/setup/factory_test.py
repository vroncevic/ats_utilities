...
class TestBaseFactory(unittest.TestCase):
    ...
    def test_create_bundle_with_generator(
        ...
        """Test complete orchestration of external pipelines when use_generator is True."""
        ...
        mock_info_fac.create_bundle.assert_called_once()
        mock_info_cls.assert_called_once_with(own=mock_info_bundle)

        mock_splash_fac.create_bundle.assert_called_once()
        mock_splash_cls.assert_called_once_with(own=mock_splash_bundle)

        mock_opt_fac.create_bundle.assert_called_once()
        mock_opt_cls.assert_called_once_with(own=mock_opt_bundle)

        mock_gen_fac.create_bundle.assert_called_once()
        mock_gen_cls.assert_called_once_with(own=mock_generator_bundle)
    ...
    def test_create_bundle_without_generator(
        ...
        # GeneratorManager boundary configurations (should not be called)
        mock_gen_fac.create_bundle.assert_not_called()
